...
class BookingsController extends Controller
{
    ...
    {
        $bikes = Bookings::select_single($id); 
		...
		if (Request::isMethod('post'))
		{
			...
			try 
			{
				...
				Bookings::soft_delete($deleteid, $form_data['modified_by'], $form_data['modified_on']);
				

				$msg=array(
					'type'=>'Success',
					'code'=>1,
					'text'=>'Booking(s) deleted sucessfully');
			} 
			catch (\Exception  $e)
			{
				DB::rollback();
		...
		{ 
			 // load the view and pass the variables
			return View::make('bookings.form_delete')
			->with('action','delete')
